...
try {
  ...
  if(layout==='team') {
    ...
    const expectedSkills=['collaborative-foundation-infra','teamai-cli','ric-devflow','ric-devflow-planner','ric-devflow-reviewer','ric-devflow-tester','ric-devflow-implementer','ric-design-patterns-skill','team-wiki-codebase','teamai-share-learnings'].sort();
    if(!Array.isArray(receipt.skills) || JSON.stringify([...receipt.skills].sort())!==JSON.stringify(expectedSkills) || !Array.isArray(receipt.upstreams) || JSON.stringify(receipt.upstreams.map(item=>item.id).sort())!==JSON.stringify(['ric-design-patterns','ric-devflow','teamai-cli']) || receipt.upstreams.some(item=>item.path!==`skills/upstreams/${item.id}` || !/^[a-f0-9]{40}$/.test(item.commit)))throw new Error('Trusted receipt lacks the complete ten-Skill, three-source identity');
