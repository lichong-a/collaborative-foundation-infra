...
先遵循 [工程规范与会话协议](../collaborative-foundation-infra/SKILL.md)，确认当前模式、任务和允许写入范围。分发十包不表示调用这些方法，也不证明宿主已加载。普通开发不自动生成知识库或分享经验，basic 不准备 DevFlow 原生四角色。
...
- 官方内容许可：[MIT LICENSE](../../upstreams/teamai-cli/LICENSE)。八份上游包保持原字节，不改它们的原生配置与角色职责。
