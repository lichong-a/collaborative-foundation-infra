#!/usr/bin/env node
import { createRequire as __collaborativeFoundationInfraCreateRequire } from 'node:module'; const require = __collaborativeFoundationInfraCreateRequire(import.meta.url);
...
var OMIT_DIRS = /* @__PURE__ */ new Set([".git", "node_modules", ".venv", "__pycache__", ".cache", ".worktree", ".worktrees"]);
...
var digest = (value) => hash(stable(value));
var exists = (file) => {
  try {
    fs.lstatSync(file);
    return true;
...
var under = (file, prefix) => prefix === "." || file === prefix || file.startsWith(`${prefix}/`);
function relativeName(name, allowRoot = false) {
...
function inventory(root, { skipDirectories = OMIT_DIRS } = {}) {
  const files = {};
  function visit(directory, prefix = "") {
    for (const entry of fs.readdirSync(directory, { withFileTypes: true }).sort((a, b) => a.name.localeCompare(b.name, "en"))) {
      ...
      } else if (entry.isSymbolicLink()) files[name] = { type: "symlink", target: fs.readlinkSync(full), mode: stat.mode & 511 };
      else if (entry.isFile()) {
        if (stat.size > 128 * 1024 * 1024) throw new Error(`Snapshot file exceeds 128 MiB: ${name}`);
        files[name] = { type: "file", hash: hash(fs.readFileSync(full)), mode: stat.mode & 511 };
...
function git(root, args, optional = false) {
  try {
    return execFileSync("git", ["-C", root, ...args], { encoding: "utf8", env: { ...process.env, GIT_OPTIONAL_LOCKS: "0" }, stdio: ["ignore", "pipe", "pipe"], maxBuffer: 20 * 1024 * 1024 }).trimEnd();
  } catch (error) {
    if (optional) return null;
    throw new Error(`Git failed: ${args.join(" ")}: ${error.stderr?.toString().trim() || error.message}`);
  }
}
var head = (root) => git(root, ["rev-parse", "--verify", "HEAD"], true);
function assertCleanTarget(root, name) {
...
function readText(root, name, overrides = /* @__PURE__ */ new Map()) {
...
var fields = /* @__PURE__ */ new Set(["schemaVersion", "documentRoots", "entrypoints", "metadataRoots", "exclude", "allowedTopLevel", "protectedPaths", "pathRules", "debt", "indexEntries", "pendingFile", "linkRepairs", "workflow"]);
var base = { documentRoots: ["docs"], entrypoints: ["README.md"], metadataRoots: ["docs"], exclude: [], allowedTopLevel: [], protectedPaths: [], pathRules: [], debt: [], indexEntries: [], pendingFile: "docs/pending-classification.md", linkRepairs: [], workflow: { authority: "none", references: [] } };
function validatePolicy(input) {
  ...
  for (const entry of policy.indexEntries) {
    relativeName(entry.index);
    relativeName(entry.target);
...
function loadPolicy(file) {
  ...
  return validatePolicy(JSON.parse(fs2.readFileSync(file, "utf8")));
}
function protectedPath(name, policy) {
...
function assertWritable(name, policy) {
  relativeName(name);
...
var markdown = new MarkdownItCallable("commonmark", { html: true }).enable(["table", "strikethrough"]);
function slug(text2) {
  return text2.toLowerCase().replace(/<[^>]*>/g, "").replace(/[^\p{L}\p{N}\p{M}\s_-]/gu, "").replace(/\s/g, "-");
}
function parseDocument(content) {
  ...
  if (front) {
    try {
      metadata = import_yaml.default.parse(front[1], { uniqueKeys: true });
    } catch (error) {
  ...
  const links = [], anchors = /* @__PURE__ */ new Set(), generatedAnchors = /* @__PURE__ */ new Set(), used = /* @__PURE__ */ new Map();
  const html = (source) => {
  ...
  tokens.forEach((token, index) => {
    if (token.type === "heading_open") {
      ...
      const label = inline2.children?.map((child) => child.type === "html_inline" ? "" : child.content).join("") ?? inline2.content;
      const base2 = slug(label);
      let count = used.get(base2) ?? 0, candidate = count ? `${base2}-${count}` : base2;
      while (generatedAnchors.has(candidate)) {
        count++;
        candidate = `${base2}-${count}`;
      }
      used.set(base2, count + 1);
      generatedAnchors.add(candidate);
...
function resolveLink(root, source, target) {
  ...
  const relative = destination.startsWith("/") ? destination.slice(1) : path2.posix.join(path2.posix.dirname(source), destination || path2.posix.basename(source));
  let normalized = path2.posix.normalize(relative).replace(/\/+$/, "") || ".";
  ...
  try {
    ...
    if (exists(absolute) && fs3.statSync(absolute).isDirectory()) {
      const entry = ["README.md", "index.md"].find((name) => exists(path2.join(absolute, name)));
      if (!entry) return { path: normalized, anchor, directory: true, exists: true };
      normalized = path2.posix.join(normalized, entry);
      absolute = safePath(root, normalized);
    }
...
function replaceLink(content, oldTarget, newTarget) {
  ...
  const before = document.links.map((link2) => link2 === oldNormalized ? newNormalized : link2).sort();
  const after = parseDocument(proposed).links.sort();
  const expectedCount = document.links.filter((link2) => link2 === oldNormalized).length;
...
// tools/audit.mjs
function audit(root, policy, options = {}) {
  const files = options.files ?? inventory(root), overrides = options.overrides ?? /* @__PURE__ */ new Map();
  const findings = [];
  function add(code2, file, message, target = "") {
    const fingerprint = digest({ code: code2, path: file, target });
    findings.push({ code: code2, path: file, target, message, fingerprint, debt: policy.debt.includes(fingerprint) });
  }
  ...
  const ids = /* @__PURE__ */ new Map(), edges = /* @__PURE__ */ new Map();
  for (const [name, document] of documents) {
    ...
    for (const target of document.links) {
      ...
      if (resolved.error || !resolved.exists && !overrides.has(resolved.path)) {
        add("BROKEN_LINK", name, resolved.error ?? "Link target does not exist", target);
        continue;
      }
  ...
  const visited = /* @__PURE__ */ new Set(), queue = [...policy.entrypoints];
  while (queue.length) {
  ...
  if (policy.allowedTopLevel.length) {
    for (const name of [...new Set(Object.keys(files).map((file) => file.split("/")[0]))].sort()) if (!policy.allowedTopLevel.includes(name)) add("UNREGISTERED_ROOT", name, "Top-level path has no registered responsibility");
  }
  for (const name of Object.keys(files)) for (const rule of policy.pathRules) if (under(name, rule.prefix) && !rule.allowedExtensions.includes(path3.extname(name))) add("PATH_RULE", name, "File extension violates explicit path rule", rule.prefix);
  ...
  for (const reference2 of policy.workflow.references) if (!exists(path3.join(root, reference2))) add("FLOW_CONFLICT", reference2, "Workflow authority reference does not exist");
...
function markdownLink(source, target, label) {
  const relative = path4.posix.relative(path4.posix.dirname(source), target);
...
function operationsFor(root, policy, files, overrides) {
  const proposed = /* @__PURE__ */ new Map(), originals = /* @__PURE__ */ new Map();
  ...
  for (const entry of policy.indexEntries) {
    ...
    const target = resolveLink(root, entry.index, path4.posix.relative(path4.posix.dirname(entry.index), entry.target));
    if (!target.exists || target.error) throw new Error(`Index target must exist: ${entry.target}`);
    const linked = parseDocument(content).links.some((link2) => resolveLink(root, entry.index, link2).path === entry.target);
  ...
  if (orphans.length) {
    ...
    if (!parseDocument(index).links.some((link2) => resolveLink(root, entrypoint, link2).path === policy.pendingFile)) put(entrypoint, appendEntries(index, [markdownLink(entrypoint, policy.pendingFile, "\u5F85\u5206\u7C7B\u6587\u6863")], "navigation"));
  }
  return [...proposed].filter(([name, content]) => content !== originals.get(name)).sort(([a], [b]) => a.localeCompare(b, "en")).map(([name, content]) => ({ path: name, beforeContent: originals.get(name), beforeHash: originals.get(name) === null ? null : hash(originals.get(name)), afterHash: hash(content), mode: files[name]?.mode ?? 420, content }));
}
function composePlan(root, policy, baseline, baselineHead, overrides = /* @__PURE__ */ new Map()) {
...
function createPlan(root, policy) {
  return composePlan(root, policy, inventory(root), head(root));
}
function applyPlan(root, policy, plan, hooks = {}) {
  ...
  for (const operation of plan.operations) {
    ...
    if (exists(target) && fs4.lstatSync(target).nlink !== 1) throw new Error(`Hard-linked target refused: ${operation.path}`);
  }
  ...
  const transaction = fs4.mkdtempSync(path4.join(os.tmpdir(), "collaborative-foundation-infra-transaction-"));
  fs4.chmodSync(transaction, 448);
  fs4.writeFileSync(path4.join(transaction, "plan.json"), JSON.stringify(plan, null, 2), { mode: 384 });
  const changed = [], attempted = [], owned = /* @__PURE__ */ new Set(), createdDirectories = [];
  let lock;
  const lockPath = path4.join(os.tmpdir(), `collaborative-foundation-infra-${hash(root)}.lock`);
  try {
    lock = fs4.openSync(lockPath, "wx", 384);
    fs4.writeFileSync(lock, JSON.stringify({ pid: process.pid, repo: root, transaction }));
    for (const operation of plan.operations) {
      hooks.beforeWrite?.(operation, changed.length);
      if (head(root) !== plan.head) throw new Error("Git HEAD changed during apply");
      const target = safePath(root, operation.path), actual = exists(target) ? fs4.readFileSync(target) : null;
      if (exists(target) && fs4.lstatSync(target).nlink !== 1) throw new Error(`Hard-linked target refused: ${operation.path}`);
      ...
      let directory = path4.dirname(target);
      const missing = [];
      ...
      fs4.writeFileSync(path4.join(transaction, "progress.json"), JSON.stringify({ status: "applying", attempted, changed }), { mode: 384 });
      const descriptor = fs4.openSync(target, fs4.constants.O_WRONLY | fs4.constants.O_NOFOLLOW | (actual === null ? fs4.constants.O_CREAT | fs4.constants.O_EXCL : 0), operation.mode);
      try {
        if (fs4.fstatSync(descriptor).nlink !== 1) throw new Error(`Hard-linked opened target refused: ${operation.path}`);
        if (actual !== null) {
          const opened = fs4.fstatSync(descriptor), live = fs4.lstatSync(target);
          if (opened.ino !== live.ino || opened.dev !== live.dev || hash(fs4.readFileSync(target)) !== operation.beforeHash) throw new Error(`Target replaced during apply: ${operation.path}`);
        }
        owned.add(operation.path);
        fs4.writeFileSync(descriptor, operation.content);
        fs4.ftruncateSync(descriptor, Buffer.byteLength(operation.content));
        fs4.fsyncSync(descriptor);
      } finally {
        fs4.closeSync(descriptor);
      }
      changed.push(operation.path);
      fs4.writeFileSync(path4.join(transaction, "progress.json"), JSON.stringify({ status: "applying", changed }), { mode: 384 });
      if (hash(fs4.readFileSync(safePath(root, operation.path))) !== operation.afterHash) throw new Error(`Post-write verification failed: ${operation.path}`);
    }
    if (digest(inventory(root)) !== plan.afterSnapshot) throw new Error("Repository changed during apply");
    fs4.writeFileSync(path4.join(transaction, "progress.json"), JSON.stringify({ status: "applied", changed }), { mode: 384 });
    ...
    for (const name of [...attempted].reverse()) {
      ...
      try {
        ...
        if (!owned.has(name)) {
          if (exists(target)) conflicts.push(name);
          continue;
        }
        if (!exists(target)) {
          if (operation.beforeContent !== null) conflicts.push(name);
          continue;
        }
        if (fs4.lstatSync(target).nlink !== 1) {
          conflicts.push(name);
          continue;
        }
        const actualHash = hash(fs4.readFileSync(target));
        if (actualHash === operation.beforeHash) {
          recovered.push(name);
          continue;
        }
        if (actualHash !== operation.afterHash) {
          conflicts.push(name);
          continue;
        }
        if (operation.beforeContent === null) fs4.unlinkSync(target);
        else fs4.writeFileSync(target, operation.beforeContent);
        recovered.push(name);
      } catch {
        conflicts.push(name);
      }
    ...
    fs4.writeFileSync(path4.join(transaction, "progress.json"), JSON.stringify({ status: "failed", attempted, changed, recovered, conflicts, error: error.message }), { mode: 384 });
...
function absolutePath(value, label) {
  if (typeof value !== "string" || !value || !path5.isAbsolute(value) || /[\0\r\n]/.test(value) || value.split(path5.sep).includes("..")) throw new Error(`${label} must be an absolute canonical path`);
  const absolute = path5.resolve(value);
  let current = path5.parse(absolute).root;
  for (const part of absolute.slice(current.length).split(path5.sep).filter(Boolean)) {
...
function projectIdentity(root) {
  try {
    const common = execFileSync2("git", ["-C", root, "rev-parse", "--path-format=absolute", "--git-common-dir"], { encoding: "utf8", env: { ...process.env, GIT_OPTIONAL_LOCKS: "0", LC_ALL: "C" }, stdio: ["ignore", "pipe", "pipe"] }).trim();
    ...
    let directory = root;
    while (true) {
      if (exists(path5.join(directory, ".git"))) throw new Error("Invalid Git metadata; project identity cannot be established");
...
function context(options, environment) {
  ...
  if (options.rootSessionId !== void 0) assertIdentifier(options.rootSessionId, "--root-session-id");
  const root = repositoryRoot(options.repo), xdg = environment.stateHome;
  const state = absolutePath(xdg === void 0 ? path5.join(environment.userHome, ".local", "state") : xdg, "State home");
  const identity = { project: projectIdentity(root), agent: options.agent, rootSession: hash(options.rootSessionId ?? options.sessionId) };
  const namespace = path5.join(state, "collaborative-foundation-infra"), directory = path5.join(namespace, "sessions", identity.project, identity.agent);
  const filename = path5.join(directory, `${identity.rootSession}.json`), lockfile = path5.join(directory, `${identity.rootSession}.lock`);
  return { identity, namespace, directory, filename, manualFilename: filename.replace(/\.json$/, ".manual.json"), lockfile, inherited: options.rootSessionId !== void 0, version: environment.standardVersion ?? installedVersion() };
}
function privateDirectory(directory, create = false) {
  ...
  if (!exists(directory)) {
    ...
    fs5.mkdirSync(directory, { mode: 448 });
  }
  const stat = fs5.lstatSync(directory);
  if (!stat.isDirectory() || stat.uid !== process.getuid() || (stat.mode & 63) !== 0) throw new Error("Session namespace must be private and owned by this user");
  return true;
}
function checkDirectories(ctx, create = false) {
  absolutePath(ctx.directory, "Session directory");
  if (create) {
    const state = path5.dirname(ctx.namespace);
  ...
  const relative = path5.relative(ctx.namespace, ctx.directory).split(path5.sep);
  let directory = ctx.namespace;
  if (!privateDirectory(directory, create)) return false;
  for (const part of relative) {
    directory = path5.join(directory, part);
    if (!privateDirectory(directory, create)) return false;
  }
  return true;
}
function readPrivate(filename) {
  ...
  const stat = fs5.lstatSync(filename);
  if (!stat.isFile() || stat.nlink !== 1 || stat.uid !== process.getuid() || (stat.mode & 511) !== 384 || stat.size > 1024 * 1024) throw new Error("Session file must be an ordinary private 0600 file, without hard links");
  const descriptor = fs5.openSync(filename, fs5.constants.O_RDONLY | fs5.constants.O_NOFOLLOW);
  try {
    const opened = fs5.fstatSync(descriptor);
    if (opened.ino !== stat.ino || opened.dev !== stat.dev || opened.nlink !== 1) throw new Error("Session file changed while opening");
    return { bytes: fs5.readFileSync(descriptor), stat };
  } finally {
...
function syncDirectory(directory) {
  const fd = fs5.openSync(directory, fs5.constants.O_RDONLY | fs5.constants.O_DIRECTORY);
  try {
    fs5.fsyncSync(fd);
  } finally {
    fs5.closeSync(fd);
...
function sessionAction(options, environment = {}) {
  environment = { userHome: os2.homedir(), stateHome: process.env.XDG_STATE_HOME, ...environment };
  if (!["status", "select", "switch"].includes(options.action)) throw new Error("session action must be status, select, or switch");
  if (options.action === "status" && options.mode !== void 0) throw new Error("status does not accept --mode");
  if (options.action !== "status" && !MODES.includes(options.mode)) throw new Error("--mode must be devflow or basic");
  if (options.action !== "status" && options.rootSessionId !== void 0) throw new Error("Child agents may only query and inherit the root session choice");
  ...
  if (options.action === "status") {
    ...
    if (exists(ctx.lockfile)) {
      readPrivate(ctx.lockfile);
      throw new Error("Session update in progress or interrupted; inspect its private lock before recovery");
    }
  ...
  const lock = fs5.openSync(ctx.lockfile, fs5.constants.O_WRONLY | fs5.constants.O_CREAT | fs5.constants.O_EXCL | fs5.constants.O_NOFOLLOW, 384);
  let temporary, publicationStarted = false, completed = false;
  try {
    fs5.writeFileSync(lock, JSON.stringify({ pid: process.pid, identity: ctx.identity, operation: options.action }));
    fs5.fsyncSync(lock);
    ...
    if (record && record.mode === options.mode && record.standardVersion === ctx.version) {
      completed = true;
      return response(ctx, record, "unchanged");
    }
    const event = { action: record ? "switch" : "select", mode: options.mode, selectedAt: (/* @__PURE__ */ new Date()).toISOString(), standardVersion: ctx.version };
    ...
    temporary = path5.join(ctx.directory, `.${ctx.identity.rootSession}-${randomUUID()}.tmp`);
    const descriptor = fs5.openSync(temporary, fs5.constants.O_WRONLY | fs5.constants.O_CREAT | fs5.constants.O_EXCL | fs5.constants.O_NOFOLLOW, 384);
    try {
      fs5.writeFileSync(descriptor, bytes);
    ...
    if (before) fs5.renameSync(temporary, ctx.filename);
    else {
      fs5.linkSync(temporary, ctx.filename);
    ...
    completed = true;
    return response(ctx, next, record ? "switched" : "selected");
  } finally {
    fs5.closeSync(lock);
    if (completed || !publicationStarted) {
      if (temporary && exists(temporary)) fs5.unlinkSync(temporary);
      fs5.unlinkSync(ctx.lockfile);
    }
  }
}
...
function main(argv = process.argv.slice(2)) {
  let format2 = "text";
  try {
    if (argv[0] === "session") format2 = "json";
    ...
    if (argv[0] === "session") {
      const [, action2, ...args2] = argv, options2 = { action: action2 }, keys = { "--repo": "repo", "--agent": "agent", "--session-id": "sessionId", "--root-session-id": "rootSessionId", "--mode": "mode" };
      for (let index = 0; index < args2.length; index += 2) {
        const key = keys[args2[index]];
        if (!key || !args2[index + 1] || args2[index + 1].startsWith("--") || Object.hasOwn(options2, key)) throw new Error(`Invalid session argument: ${args2[index]}`);
        options2[key] = args2[index + 1];
      }
      const report2 = sessionAction(options2);
      console.log(JSON.stringify(report2, null, 2));
      return report2.status === "awaiting-selection" ? 3 : 0;
    }
    ...
    format2 = options["--format"] ?? (action === "plan" ? "json" : "text");
    if (!["json", "text"].includes(format2)) throw new Error("format must be json or text");
    ...
    if (action === "plan") report = createPlan(root, policy);
      ...
      report = applyPlan(root, policy, JSON.parse(fs6.readFileSync(options["--plan"], "utf8")));
    } else report = audit(root, policy);
    if (format2 === "json" || action === "plan") console.log(JSON.stringify(report, null, 2));
    else {
    ...
    console.log(format2 === "json" ? JSON.stringify(report) : `error: ${error.message}`);
    return 2;
  }
}
if (process.argv[1] && path6.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) process.exitCode = main();
export {
  main
};
